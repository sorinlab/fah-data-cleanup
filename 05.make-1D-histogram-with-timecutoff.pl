...
########## global variables ####################
our $usage = "Usage: $0 [input] [X-column] [X-min] [X-max] [X-resolution] [time-cutoff(ps)] [output]";
...
our $cutoff = $ARGV[5] || die "$usage\n";
our $output = $ARGV[6] || die "$usage\n";
